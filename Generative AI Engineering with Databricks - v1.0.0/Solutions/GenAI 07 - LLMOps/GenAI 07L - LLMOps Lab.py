...
dataset = load_dataset(
    "squad", version="1.2.0", cache_dir=DA.paths.datasets
...
signature = mlflow.models.infer_signature(
    input_example, 
    mlflow.transformers.generate_signature_output(qa_pipeline, input_example)
    )

# Start the run
with mlflow.start_run(run_name = "LLMOps_Lab_QA_Pipeline"):
    ...
    model_info = mlflow.transformers.log_model(
        ...
        artifact_path="qa_pipeline_artifacts",
...
staging_model = mlflow.pyfunc.load_model(model_uri=f"models:/{model_name}/Staging")

# use dataset for inference
predictions = staging_model.predict(val_dataset)
