...
# MAGIC * You must install the **Weaviate connector**.
# MAGIC * You must install the **Weaviate client**.
# MAGIC
...
# MAGIC %md
# MAGIC ### Installing the Weaviate Connector
# MAGIC
# MAGIC Since we will be interacting with Spark by writing a Spark dataframe out to Weaviate, we need a Spark Connector. You need to attach a Spark-Weaviate connector to your cluster using the steps below. For more information, refer to this [documentation](https://docs.pinecone.io/docs/databricks#setting-up-a-spark-cluster) and [Weaviate installation instructions](https://github.com/weaviate/spark-connector#using-the-jar-in-databricks).
# MAGIC
# MAGIC
# MAGIC **Steps to follow:**
# MAGIC
# MAGIC 1. [Download the Spark connector jar file](https://github.com/weaviate/spark-connector#download-jar-from-github)
# MAGIC 1. Open the configuration page for your cluster.
# MAGIC 1. In the **Libraries** tab, click **Install New**
# MAGIC 1. For **Libary Source**, choose *DBFS*.
# MAGIC 1. For **Libary Type**, choose *JAR*.
# MAGIC 1. Drop the downloaded `jar` file to the upload area. 
...
# MAGIC
# MAGIC Before we can proceed, you need your own Weaviate Network. To start your own network, visit the [homepage](https://weaviate.io/). 
# MAGIC
...
# MAGIC %md
# MAGIC We will use embeddings from the OpenAI,  so we will need a token from OpenAI API
# MAGIC
# MAGIC Steps:
...
os.environ["WEAVIATE_NETWORK"] = dbutils.secrets.get("llm_scope", "weaviate_network")
...
# MAGIC
# MAGIC **IMPORTANT!!** Since we are writing a Spark DataFrame out, we need a Spark Connector to Weaviate. You need to [download the Spark connector jar file](https://github.com/weaviate/spark-connector#download-jar-from-github) and [upload it to your Databricks cluster](https://github.com/weaviate/spark-connector#using-the-jar-in-databricks) before running the next cell. If you do not do this, the next cell *will fail*.
