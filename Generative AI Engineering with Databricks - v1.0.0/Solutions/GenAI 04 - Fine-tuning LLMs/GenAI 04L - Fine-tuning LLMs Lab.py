...
# MAGIC
# MAGIC Before starting the lab, run the provided classroom setup script. This script will define configuration variables necessary for the demo. Execute the following cells to setup the classroom environment.
...
# ANSWER
ds = load_dataset("databricks/databricks-dolly-15k", cache_dir=DA.paths.datasets)
...
tokenizer = AutoTokenizer.from_pretrained(model_checkpoint, cache_dir=DA.paths.datasets)
...
split_dataset = tokenized_dataset["train"].train_test_split(
    train_size=TRAINING_SIZE, seed=SEED
)
trainer = Trainer(
    model,
    training_args,
...
# MAGIC
# MAGIC Run the following cell to remove lessons-specific assets created during this lab.
