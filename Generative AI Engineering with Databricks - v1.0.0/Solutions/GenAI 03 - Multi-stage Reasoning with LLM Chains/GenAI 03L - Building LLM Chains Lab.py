...
# MAGIC %md
# MAGIC
# MAGIC # Adding Our Own Data to a Multi-Stage Reasoning System - Lab
# MAGIC
...
os.environ["HUGGINGFACEHUB_API_TOKEN"] = dbutils.secrets.get(
    "llm_scope", "huggingface_key"
)
...
chromadb_index = Chroma.from_documents(
    texts, embeddings, persist_directory=tmp_shakespeare_dir.name
)
...
qa = RetrievalQA.from_chain_type(
    llm=hf_llm, chain_type="stuff", retriever=chromadb_index.as_retriever()
)
...
qa = RetrievalQA.from_chain_type(
    llm=hf_llm, chain_type="map_reduce", retriever=chromadb_index.as_retriever()
)
...
qa = RetrievalQA.from_chain_type(
    llm=hf_llm, chain_type="refine", retriever=chromadb_index.as_retriever()
)
...
# MAGIC ## Summary
# MAGIC
# MAGIC At the conclusion of this lab, you have acquired the skills to enrich your language model's knowledge base by integrating external data via vector databases. You have also mastered the construction of Question-Answer LLMChains to interact with this extended knowledge base. Additionally, you've learned how to load data from remote sources and efficiently search through it using various retrieval methods.
